...
## 課題記録: 初回提出が Guideline 2.1 で差し戻し (2026-08)

- 症状: App Review から「Guideline 2.1 - Information Needed」。バグではなく、
  審査メモの情報不足。7項目（実機での画面収録／テスト端末一覧／機能と対象／
  使い方／外部サービス／地域差／第三者素材の権利）の提出を求められた。
- 原因: 審査メモが簡易すぎた。とくに「実機での画面収録」は必須で、
  許可ダイアログが出る場面を含める必要がある。
- 対応: `notes/APP_REVIEW_REPLY.md` に英文の回答一式を用意（次回以降も使い回す）。
  あわせて NSLocalNetworkUsageDescription を「理由＋具体例」の形に書き直した
  （Apple のガイドは purpose string に使い方の例を求めている）。
- 教訓: **審査メモは最初から詳しく書く。** Apple も「次回以降の提出でも
  Notes に入れておくこと」と明記している。ログイン不要・課金なし・UGCなしの
  アプリでも、「無い」ことを明示的に書く必要がある。
